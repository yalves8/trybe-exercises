...
function averageAge() {
    ...
    return books.reduce((acc, valor) => acc + (valor.releaseYear - valor.author.birthYear), 0) / tamanho;
}
...
function longestNamedBook() {
    // escreva seu código aqui
    return books.reduce((grande, atual) => {
        if (atual.name.length > grande.name.length) return atual;
        return grande;
    });
}
